...
$sql = "SELECT count(id) as sa,marka FROM produktet group by marka";
foreach($conn->query($sql) as $row) { 
	$arr[$row["marka"]]=$row["sa"];
}	
...
<tr>
		...
		<?php
		$i=0;
		$sql = "SELECT * FROM marka ORDER BY radha asc";
		foreach($conn->query($sql) as $row) { 
		?>
			<tr id="m_<?php echo $row['id']?>"  <?php echo $shto;?>>
                ...
                <td><?php echo $row['marka'];?> (<?php echo $arr[$row['marka']];?>)</td>
 ...
 <a href="javascript:fshi('<?php echo $row['id'];?>','marka')" class="btn btn-danger btn-xs"><i title="Delete" class="ikon icon-close"></i></a>
...
<script>
    $("#sortable").sortable({
      ...
      update : function () {
		...
		$.post("functions/database/rendit.php?t=marka&"+order,  function( data ) {
		});
...
var tabela=$('#tabela').DataTable({
	...
	function shto() {
		bejmodal("Shto markë","functions/marka/edit.php",ndrysho1);
	}
	
	function ndrysho(id) {
		bejmodal("Ndrysho markë","functions/marka/edit.php?id=" +id,ndrysho1);
	}
	


	function ndrysho1(){
		$.post( "functions/marka/query.php", $( "#ndrysho" ).serialize(), function( data ) {
			location.reload();
		});
	}
