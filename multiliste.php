...
<tr>
		...
		<?php
		$i=0;
		$sql = "SELECT * FROM multi {$kerko} ORDER BY id DESC";
...
<a href="multi.php?id=<?php echo $row['id'];?>" class="btn btn-info btn-xs" ><i title="Popullo" class="ikon icon-eye"></i></a>

 <a href="javascript:ndrysho('<?php echo $row['id'];?>')" class="btn btn-warning btn-xs" ><i title="Edit" class="ikon icon-pencil"></i></a>

 <a href="javascript:fshi('<?php echo $row['id'];?>','multi')" class="btn btn-danger btn-xs"><i title="Delete" class="ikon icon-close"></i></a>
...
var tabela=$('#tabela').DataTable({
	...
	function shto() {
		bejmodal("Shto koleksion","functions/multi/edit.php",ndrysho1);
	}
	
	function ndrysho(id) {
		bejmodal("Ndrysho koleksion","functions/multi/edit.php?id=" +id,ndrysho1);
	}
	


	function ndrysho1(){
		$.post( "functions/multi/query.php", $( "#ndrysho" ).serialize(), function( data ) {
			location.reload();
		});
	}
